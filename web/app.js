...
function renderInspector() {
  ...
  if (!node) {
    ...
    const overview = comparisons.length
      ...
      : "";
    const quickStarts = [
      ["irrational-sqrt-two", "Start concrete · irrationality of √2"],
      ["fermat-prime-two-squares", "Compare arguments · Fermat’s two-square theorem"],
      ["cosine-addition", "Use Euler · cosine addition"],
    ].map(([id, label]) => comparisons.some((comparison) => comparison.id === id)
      ? `<a href="?comparison=${encodeURIComponent(id)}">${escapeHtml(label)}</a>` : "").filter(Boolean).join("");
    const readerGuide = `<section class="reader-guide"><div class="detail-label">How to read a proof landscape</div>${quickStarts ? `<div class="quick-starts"><span>Suggested first landscapes</span>${quickStarts}</div>` : ""}<p class="journey-link"><a href="https://github.com/liuyao12/math-net/blob/main/docs/reader-journeys.md" target="_blank" rel="noreferrer">Read the guided journeys ↗</a></p><ol><li>Choose a concrete problem or comparison. The selected result stays low in the graph.</li><li>Read upward: every arrow means the upper declaration is <em>used in the Lean proof</em> of the lower one.</li><li>Colors identify the repository supplying a proof route. Select a route to see its own mathematical prerequisites.</li><li>Click any node for its checked statement and source; gold-outlined squares are familiar mathematical foundations, while faded nodes are formal plumbing.</li></ol><details class="reading-glossary"><summary>Words used here</summary><dl><dt>Declaration</dt><dd>Any named Lean object: a theorem, definition, structure, or lemma. The node shape and inspector identify its kind.</dd><dt>Proof route</dt><dd>One checked Lean proof body for a result. Routes may come from different repositories and can be selected independently.</dd><dt>Black box</dt><dd>A displayed result whose further prerequisites are presently collapsed—not an unproved assertion. Use its + marker, double-click it, or choose “Reveal Lean construction.”</dd><dt>Exact merge</dt><dd>Two routes conclude precisely the same Lean proposition, so Lean can check both against one shared node.</dd><dt>Foundation-aligned</dt><dd>Routes express the same intended mathematics over different constructions, such as ordinary reals and computable reals. They remain separate until a checked bridge identifies them.</dd></dl></details></section>`;
...
function draw() {
  ...
  node.attr("tabindex", 0).on("keydown", (event, item) => {
    if (event.key !== "Enter" && event.key !== " ") return;
    event.preventDefault();
    event.stopPropagation();
    selectNode(item.id);
  });
  node.append("title").text((item) => `${item.namespace || displayLabelFor(item)}${item.readerNote ? `\n${item.readerNote.title}: ${item.readerNote.statement}` : ""}\nClick or press Enter to inspect this checked declaration.`);
